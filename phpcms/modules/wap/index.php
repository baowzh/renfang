...
class index {
	...
	public function main() {
		...
		$WAP_SETTING = string2array ( $WAP ['setting'] );
		$GLOBALS ['siteid'] = $siteid = max ( $this->siteid, 1 );
		$template = $WAP_SETTING ['main_template'] ? $WAP_SETTING ['main_template'] : 'loading';
		include template ( 'wap', $template );
	}
	...
	public function lists() {
		...
		$total = $this->db->count ( array (
				'status' => '99',
				'catid' => $catid 
		) );
		$page = isset ( $_GET ['page'] ) && intval ( $_GET ['page'] ) ? intval ( $_GET ['page'] ) : 1;
		$pagesize = $WAP_SETTING ['listnum'] ? intval ( $WAP_SETTING ['listnum'] ) : 3;
		$offset = ($page - 1) * $pagesize;
		
		$list = $this->db->select ( array (
				'status' => '99',
				'catid' => $catid 
	...
	function comment_list() {
		...
		if (! preg_match ( "/^[a-z0-9_\-]+$/i", $commentid ))
		...
		$setting = $comment_setting_db->get_one ( array (
				'siteid' => $this->siteid 
		) );
	...
	public function search() {
		...
		$search_q = $q; // 搜索原内容
		$time = empty ( $_GET ['time'] ) || ! in_array ( $_GET ['time'], array (
				'all',
				'day',
				'month',
				'year' 

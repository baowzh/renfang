...
<div class="pad_10">
	<form action="?m=vote&c=vote&a=edit&subjectid=<?php echo $subjectid?>"
		...
		<table cellpadding="2" cellspacing="1" class="table_form" width="100%">
			...
			<tr id="SizeFormat" <?php if($ischeckbox == '0'){ ?>
				style="display: none" <?php }else{?> style="display:" <?php }?>>
				<th></th>
				<td><label><?php echo L('minval');?></label>&nbsp;&nbsp;<input
					name="subject[minval]" class="input-text" type="text" size="5"
					value="<?php echo $minval;?>"> <?php echo L('item')?> &nbsp;&nbsp;&nbsp;&nbsp; <label><?php echo L('maxval');?></label>&nbsp;&nbsp;<input
					name="subject[maxval]" type="text" class="input-text" size="5"
					value="<?php echo $maxval?>"> <?php echo L('item')?></td>
			</tr>
    ...
    <?php
				...
				foreach ( $options as $optionid => $option ) {
					$i ++;
					?>
           <div id="option<?php echo $option['optionid'];?>">
							<br> <input type="text"
								name="option[<?php echo $option['optionid']?>]" size="40"
								require="true" value="<?php echo $option['option'];?>" />
           <?php if($i>2){?>
           <input type="button" value="<?php echo L('del')?>"
								onclick="del_old(<?php echo $option['optionid'];?>)"
								class="button" />
           <?php }?> 
	...
	<?php }?>
			...
			<tr>
				...
				<td><input name="subject[allowview]" type="radio" value="1"
					<?php echo $allowview?'checked':''?>>&nbsp;<?php echo L('allow')?>&nbsp;&nbsp;<input
					name="subject[allowview]" type="radio" value="0"
					<?php echo $allowview?'':'checked'?>>&nbsp;<?php echo L('not_allow')?></td>
			</tr>
			<tr>
				<th><?php echo L('allowguest')?>：</th>
				<td><input name="subject[allowguest]" type="radio" value="1"
					<?php echo $allowguest?'checked':''?>>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;<input
					name="subject[allowguest]" type="radio" value="0"
					<?php echo $allowguest?'':'checked'?>>&nbsp;<?php echo L('no')?></td>
			</tr>
			<tr>
				<th><?php echo L('credit')?>：</th>
				<td><input type="text" name="subject[credit]"
		...
		<?php echo form::select_template('default', 'vote', $template, 'name="vote_subject[show_template]"', 'vote_tp');?>
			...
			<tr>
				...
				<td><input name="subject[enabled]" type="radio" value="1"
					<?php echo $enabled?'checked':''?>>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;<input
					name="subject[enabled]" type="radio" value="0"
					<?php echo $enabled?'':'checked'?>>&nbsp;<?php echo L('no')?></td>
			</tr>
			<tr>
				<th></th>
				<td><input type="submit" name="dosubmit" id="dosubmit"
					class="dialog" value=" <?php echo L('submit')?> "></td>
			</tr>
